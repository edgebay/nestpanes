...
def version_info_builder(target, source, env):
    with methods.generated_wrapper(str(target[0])) as file:
        file.write(
            """\
#define APP_VERSION_SHORT_NAME "{app_short_name}"
#define APP_VERSION_NAME "{app_name}"
#define APP_VERSION_MAJOR {app_major}
#define APP_VERSION_MINOR {app_minor}
#define APP_VERSION_PATCH {app_patch}
#define APP_VERSION_STATUS "{app_status}"
#define APP_VERSION_BUILD "{app_build}"
#define APP_VERSION_MODULE_CONFIG "{app_module_config}"
#define APP_VERSION_WEBSITE "{app_website}"
