...
void FileSystemTreeItem::set_custom_draw(int p_column, Object *p_object, const StringName &p_callback) {
...
void FileSystemTreeItem::clear_buttons() {
	int i = 0;
	for (Cell &cell : cells) {
...
void FileSystemTreeItem::add_button(int p_column, const Ref<Texture2D> &p_button, int p_id, bool p_disabled, const String &p_tooltip, const String &p_description) {
	ERR_FAIL_INDEX(p_column, cells.size());
	ERR_FAIL_COND(p_button.is_null());
	FileSystemTreeItem::Cell::Button button;
...
int FileSystemTreeItem::get_button_count(int p_column) const {
	ERR_FAIL_INDEX_V(p_column, cells.size(), -1);
	return cells[p_column].buttons.size();
}

Ref<Texture2D> FileSystemTreeItem::get_button(int p_column, int p_index) const {
	ERR_FAIL_INDEX_V(p_column, cells.size(), Ref<Texture2D>());
	ERR_FAIL_INDEX_V(p_index, cells[p_column].buttons.size(), Ref<Texture2D>());
	return cells[p_column].buttons[p_index].texture;
}

String FileSystemTreeItem::get_button_tooltip_text(int p_column, int p_index) const {
...
int FileSystemTreeItem::get_button_id(int p_column, int p_index) const {
	ERR_FAIL_INDEX_V(p_column, cells.size(), -1);
	ERR_FAIL_INDEX_V(p_index, cells[p_column].buttons.size(), -1);
	return cells[p_column].buttons[p_index].id;
}

void FileSystemTreeItem::erase_button(int p_column, int p_index) {
	ERR_FAIL_INDEX(p_column, cells.size());
...
int FileSystemTreeItem::get_button_by_id(int p_column, int p_id) const {
	ERR_FAIL_INDEX_V(p_column, cells.size(), -1);
...
Color FileSystemTreeItem::get_button_color(int p_column, int p_index) const {
	ERR_FAIL_INDEX_V(p_column, cells.size(), Color());
	ERR_FAIL_INDEX_V(p_index, cells[p_column].buttons.size(), Color());
	return cells[p_column].buttons[p_index].color;
}

void FileSystemTreeItem::set_button_tooltip_text(int p_column, int p_index, const String &p_tooltip) {
	ERR_FAIL_INDEX(p_column, cells.size());
	ERR_FAIL_INDEX(p_index, cells[p_column].buttons.size());
...
void FileSystemTreeItem::set_button(int p_column, int p_index, const Ref<Texture2D> &p_button) {
	ERR_FAIL_COND(p_button.is_null());
	ERR_FAIL_INDEX(p_column, cells.size());
	ERR_FAIL_INDEX(p_index, cells[p_column].buttons.size());
...
void FileSystemTreeItem::set_button_description(int p_column, int p_index, const String &p_description) {
	ERR_FAIL_INDEX(p_column, cells.size());
	ERR_FAIL_INDEX(p_index, cells[p_column].buttons.size());
...
void FileSystemTreeItem::set_button_color(int p_column, int p_index, const Color &p_color) {
	ERR_FAIL_INDEX(p_column, cells.size());
	ERR_FAIL_INDEX(p_index, cells[p_column].buttons.size());
...
void FileSystemTreeItem::set_button_disabled(int p_column, int p_index, bool p_disabled) {
	ERR_FAIL_INDEX(p_column, cells.size());
	ERR_FAIL_INDEX(p_index, cells[p_column].buttons.size());
...
bool FileSystemTreeItem::is_button_disabled(int p_column, int p_index) const {
	ERR_FAIL_INDEX_V(p_column, cells.size(), false);
...
void FileSystemTreeItem::set_custom_as_button(int p_column, bool p_button) {
	ERR_FAIL_INDEX(p_column, cells.size());
...
bool FileSystemTreeItem::is_custom_set_as_button(int p_column) const {
	ERR_FAIL_INDEX_V(p_column, cells.size(), false);
...
void FileSystemTreeItem::_bind_methods() {
...
#ifndef DISABLE_DEPRECATED
	ClassDB::bind_method(D_METHOD("set_custom_draw", "column", "object", "callback"), &FileSystemTreeItem::set_custom_draw);
#endif // DISABLE_DEPRECATED
	...
	ClassDB::bind_method(D_METHOD("set_custom_as_button", "column", "enable"), &FileSystemTreeItem::set_custom_as_button);
	ClassDB::bind_method(D_METHOD("is_custom_set_as_button", "column"), &FileSystemTreeItem::is_custom_set_as_button);

	ClassDB::bind_method(D_METHOD("clear_buttons"), &FileSystemTreeItem::clear_buttons);
	ClassDB::bind_method(D_METHOD("add_button", "column", "button", "id", "disabled", "tooltip_text", "description"), &FileSystemTreeItem::add_button, DEFVAL(-1), DEFVAL(false), DEFVAL(""), DEFVAL(""));
	ClassDB::bind_method(D_METHOD("get_button_count", "column"), &FileSystemTreeItem::get_button_count);
	ClassDB::bind_method(D_METHOD("get_button_tooltip_text", "column", "button_index"), &FileSystemTreeItem::get_button_tooltip_text);
	ClassDB::bind_method(D_METHOD("get_button_id", "column", "button_index"), &FileSystemTreeItem::get_button_id);
	ClassDB::bind_method(D_METHOD("get_button_by_id", "column", "id"), &FileSystemTreeItem::get_button_by_id);
	ClassDB::bind_method(D_METHOD("get_button_color", "column", "id"), &FileSystemTreeItem::get_button_color);
	ClassDB::bind_method(D_METHOD("get_button", "column", "button_index"), &FileSystemTreeItem::get_button);
	ClassDB::bind_method(D_METHOD("set_button_tooltip_text", "column", "button_index", "tooltip"), &FileSystemTreeItem::set_button_tooltip_text);
	ClassDB::bind_method(D_METHOD("set_button", "column", "button_index", "button"), &FileSystemTreeItem::set_button);
	ClassDB::bind_method(D_METHOD("erase_button", "column", "button_index"), &FileSystemTreeItem::erase_button);
	ClassDB::bind_method(D_METHOD("set_button_description", "column", "button_index", "description"), &FileSystemTreeItem::set_button_description);
	ClassDB::bind_method(D_METHOD("set_button_disabled", "column", "button_index", "disabled"), &FileSystemTreeItem::set_button_disabled);
	ClassDB::bind_method(D_METHOD("set_button_color", "column", "button_index", "color"), &FileSystemTreeItem::set_button_color);
	ClassDB::bind_method(D_METHOD("is_button_disabled", "column", "button_index"), &FileSystemTreeItem::is_button_disabled);
	...
	if (p_event->is_action(cache.rtl ? "ui_left" : "ui_right") && p_event->is_pressed()) {
		...
		if (select_mode == SELECT_MULTI) {
			...
			if (selected_item && selected_col != -1 && selected_button != -1) {
				const FileSystemTreeItem::Cell &c = selected_item->cells[selected_col];
	...
	if (mb.is_valid()) {
		...
		if (!mb->is_pressed()) {
			...
			if (cache.click_type == Cache::CLICK_BUTTON && cache.click_item != nullptr) {
				...
				FileSystemTreeItem *current_item;
...
void FileSystemTree::_check_item_accessibility(FileSystemTreeItem *p_item, PackedStringArray &r_warnings, int &r_row) const {
	for (int i = 0; i < p_item->cells.size(); i++) {
		const FileSystemTreeItem::Cell &cell = p_item->cells[i];
...
PackedStringArray FileSystemTree::get_accessibility_configuration_warnings() const {
...
void FileSystemTree::_accessibility_action_scroll_down(const Variant &p_data) {
...
void FileSystemTree::_accessibility_action_scroll_left(const Variant &p_data) {
	if ((DisplayServer::AccessibilityScrollUnit)p_data == DisplayServer::SCROLL_UNIT_ITEM) {
...
void FileSystemTree::_accessibility_action_scroll_right(const Variant &p_data) {
	if ((DisplayServer::AccessibilityScrollUnit)p_data == DisplayServer::SCROLL_UNIT_ITEM) {
		h_scroll->set_value(h_scroll->get_value() + h_scroll->get_page() / 4);
	} else {
		h_scroll->set_value(h_scroll->get_value() + h_scroll->get_page());
...
void FileSystemTree::_accessibility_action_scroll_up(const Variant &p_data) {
	if ((DisplayServer::AccessibilityScrollUnit)p_data == DisplayServer::SCROLL_UNIT_ITEM) {
		v_scroll->set_value(v_scroll->get_value() - v_scroll->get_page() / 4);
	} else {
		v_scroll->set_value(v_scroll->get_value() - v_scroll->get_page());
...
void FileSystemTree::_accessibility_action_scroll_set(const Variant &p_data) {
	const Point2 &pos = p_data;
	h_scroll->set_value(pos.x);
	v_scroll->set_value(pos.y);
}

void FileSystemTree::_accessibility_action_scroll_into_view(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
	scroll_to_item(p_item);
}

void FileSystemTree::_accessibility_action_focus(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
	p_item->select(p_col);
}

void FileSystemTree::_accessibility_action_blur(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
	p_item->deselect(p_col);
}

void FileSystemTree::_accessibility_action_collapse(const Variant &p_data, FileSystemTreeItem *p_item) {
	p_item->set_collapsed(true);
}

void FileSystemTree::_accessibility_action_expand(const Variant &p_data, FileSystemTreeItem *p_item) {
	p_item->set_collapsed(false);
}

void FileSystemTree::_accessibility_action_set_text_value(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
	p_item->set_text(p_col, p_data);
}

void FileSystemTree::_accessibility_action_set_num_value(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
	p_item->set_range(p_col, p_data);
}

void FileSystemTree::_accessibility_action_set_bool_value(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
	p_item->set_checked(p_col, !p_item->cells[p_col].checked);
}

void FileSystemTree::_accessibility_action_edit_custom(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
...
void FileSystemTree::_accessibility_action_set_inc(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
	p_item->set_range(p_col, p_item->cells[p_col].val + p_item->cells[p_col].step);
}

void FileSystemTree::_accessibility_action_set_dec(const Variant &p_data, FileSystemTreeItem *p_item, int p_col) {
	p_item->set_range(p_col, p_item->cells[p_col].val - p_item->cells[p_col].step);
}

void FileSystemTree::_accessibility_action_button_press(const Variant &p_data, FileSystemTreeItem *p_item, int p_col, int p_btn) {
	emit_signal("button_clicked", p_item, p_col, p_btn, MouseButton::LEFT);
}

RID FileSystemTree::get_focused_accessibility_element() const {
	if (selected_item) {
		if (selected_col >= 0) {
...
void FileSystemTree::_accessibility_clean_info(FileSystemTreeItem *p_item) {
	p_item->accessibility_row_element = RID();
	for (FileSystemTreeItem::Cell &cell : p_item->cells) {
		cell.accessibility_cell_element = RID();
		for (FileSystemTreeItem::Cell::Button &btn : cell.buttons) {
			btn.accessibility_button_element = RID();
		}
	}
...
void FileSystemTree::_accessibility_update_item(Point2 &r_ofs, FileSystemTreeItem *p_item, int &r_row, int p_level) {
	// Row.
	if ((p_item != root || !hide_root) && p_item->is_visible()) {
		...
		DisplayServer::get_singleton()->accessibility_update_add_action(p_item->accessibility_row_element, DisplayServer::AccessibilityAction::ACTION_COLLAPSE, callable_mp(this, &FileSystemTree::_accessibility_action_collapse).bind(p_item));
		DisplayServer::get_singleton()->accessibility_update_add_action(p_item->accessibility_row_element, DisplayServer::AccessibilityAction::ACTION_EXPAND, callable_mp(this, &FileSystemTree::_accessibility_action_expand).bind(p_item));
		...
		if (p_item->accessibility_row_dirty) {
			...
			int col_offset = 0;
			for (int i = 0; i < p_item->cells.size(); i++) {
				FileSystemTreeItem::Cell &cell = p_item->cells.write[i];
				...
				DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_SCROLL_INTO_VIEW, callable_mp(this, &FileSystemTree::_accessibility_action_scroll_into_view).bind(p_item, i));
				DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_FOCUS, callable_mp(this, &FileSystemTree::_accessibility_action_focus).bind(p_item, i));
				DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_BLUR, callable_mp(this, &FileSystemTree::_accessibility_action_blur).bind(p_item, i));
				DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_COLLAPSE, callable_mp(this, &FileSystemTree::_accessibility_action_collapse).bind(p_item));
				DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_EXPAND, callable_mp(this, &FileSystemTree::_accessibility_action_expand).bind(p_item));
				...
				switch (cell.mode) {
					case FileSystemTreeItem::CELL_MODE_STRING: {
						DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_SET_VALUE, callable_mp(this, &FileSystemTree::_accessibility_action_set_text_value).bind(p_item, i));
						DisplayServer::get_singleton()->accessibility_update_set_value(cell.accessibility_cell_element, cell.xl_text);
					} break;
					case FileSystemTreeItem::CELL_MODE_CHECK: {
						DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_CLICK, callable_mp(this, &FileSystemTree::_accessibility_action_set_bool_value).bind(p_item, i));
						DisplayServer::get_singleton()->accessibility_update_set_checked(cell.accessibility_cell_element, cell.checked);
					} break;
					case FileSystemTreeItem::CELL_MODE_RANGE: {
						DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_DECREMENT, callable_mp(this, &FileSystemTree::_accessibility_action_set_dec).bind(p_item, i));
						DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_INCREMENT, callable_mp(this, &FileSystemTree::_accessibility_action_set_inc).bind(p_item, i));
						DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_SET_VALUE, callable_mp(this, &FileSystemTree::_accessibility_action_set_num_value).bind(p_item, i));
					...
					case FileSystemTreeItem::CELL_MODE_CUSTOM: {
						DisplayServer::get_singleton()->accessibility_update_add_action(cell.accessibility_cell_element, DisplayServer::AccessibilityAction::ACTION_CLICK, callable_mp(this, &FileSystemTree::_accessibility_action_edit_custom).bind(p_item, i));
					} break;
				}
				...
				for (int j = cell.buttons.size() - 1; j >= 0; j--) {
					...
					DisplayServer::get_singleton()->accessibility_update_add_action(cell.buttons[j].accessibility_button_element, DisplayServer::AccessibilityAction::ACTION_CLICK, callable_mp(this, &FileSystemTree::_accessibility_action_button_press).bind(p_item, i, j));
...
void FileSystemTree::_notification(int p_what) {
	switch (p_what) {
		...
		case NOTIFICATION_ACCESSIBILITY_UPDATE: {
			...
			DisplayServer::get_singleton()->accessibility_update_add_action(ae, DisplayServer::AccessibilityAction::ACTION_SCROLL_DOWN, callable_mp(this, &FileSystemTree::_accessibility_action_scroll_down));
			DisplayServer::get_singleton()->accessibility_update_add_action(ae, DisplayServer::AccessibilityAction::ACTION_SCROLL_LEFT, callable_mp(this, &FileSystemTree::_accessibility_action_scroll_left));
			DisplayServer::get_singleton()->accessibility_update_add_action(ae, DisplayServer::AccessibilityAction::ACTION_SCROLL_RIGHT, callable_mp(this, &FileSystemTree::_accessibility_action_scroll_right));
			DisplayServer::get_singleton()->accessibility_update_add_action(ae, DisplayServer::AccessibilityAction::ACTION_SCROLL_UP, callable_mp(this, &FileSystemTree::_accessibility_action_scroll_up));
			DisplayServer::get_singleton()->accessibility_update_add_action(ae, DisplayServer::AccessibilityAction::ACTION_SET_SCROLL_OFFSET, callable_mp(this, &FileSystemTree::_accessibility_action_scroll_set));
...
Rect2 FileSystemTree::get_item_rect(FileSystemTreeItem *p_item, int p_column, int p_button) const {
	...
	if (p_column == -1) {
		...
		if (p_button != -1) {
			...
			const FileSystemTreeItem::Cell &c = p_item->cells[p_column];
	...
	if ((root != p_item || !hide_root) && p_item->is_visible_in_tree()) {
		...
		if (pos.y < r_height) {
			...
			for (int i = 0; i < columns.size(); i++) {
				...
				if (p_item->cells[i].expand_right) {
					int plus = 1;
					while (i + plus < columns.size() && !p_item->cells[i + plus].editable && p_item->cells[i + plus].mode == FileSystemTreeItem::CELL_MODE_STRING && p_item->cells[i + plus].text.is_empty() && p_item->cells[i + plus].icon.is_null() && p_item->cells[i + plus].buttons.is_empty()) {
...
void FileSystemTree::_find_button_at_pos(const Point2 &p_pos, FileSystemTreeItem *&r_item, int &r_column, int &r_index, int &r_section) const {
	...
	const FileSystemTreeItem::Cell &c = it->cells[col];
...
void FileSystemTree::_bind_methods() {
	...
	ADD_SIGNAL(MethodInfo("button_clicked", PropertyInfo(Variant::OBJECT, "item", PROPERTY_HINT_RESOURCE_TYPE, "FileSystemTreeItem"), PropertyInfo(Variant::INT, "column"), PropertyInfo(Variant::INT, "id"), PropertyInfo(Variant::INT, "mouse_button_index")));
