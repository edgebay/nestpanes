...
class FileSystemTreeItem : public Object {
	GDCLASS(FileSystemTreeItem, Object);
...
private:
	...
	FileSystemTreeItem *parent = nullptr; // Parent item.
	FileSystemTreeItem *prev = nullptr; // Previous in list.
	FileSystemTreeItem *next = nullptr; // Next in list.
	FileSystemTreeItem *first_child = nullptr;
	FileSystemTreeItem *last_child = nullptr;

	LocalVector<FileSystemTreeItem *> children_cache;
	bool is_root = false; // For tree root.
	FileSystemTree *tree = nullptr; // FileSystemTree (for reference).

	FileSystemTreeItem(FileSystemTree *p_tree);
	...
	void _change_tree(FileSystemTree *p_tree);
...
private:
	...
	FileSystemTreeItem *_get_prev_in_tree(bool p_wrap = false, bool p_include_invisible = false);
	FileSystemTreeItem *_get_next_in_tree(bool p_wrap = false, bool p_include_invisible = false);
...
public:
	...
	// Item manipulation.
	FileSystemTreeItem *create_child(int p_index = -1);
	void add_child(FileSystemTreeItem *p_item);
	void remove_child(FileSystemTreeItem *p_item);

	FileSystemTree *get_tree() const;

	FileSystemTreeItem *get_prev();
	FileSystemTreeItem *get_next() const;
	FileSystemTreeItem *get_parent() const;
	FileSystemTreeItem *get_first_child() const;

	FileSystemTreeItem *get_prev_in_tree(bool p_wrap = false);
	FileSystemTreeItem *get_next_in_tree(bool p_wrap = false);

	FileSystemTreeItem *get_prev_visible(bool p_wrap = false);
	FileSystemTreeItem *get_next_visible(bool p_wrap = false);

	FileSystemTreeItem *get_child(int p_index);
	int get_visible_child_count();
	int get_child_count();
	TypedArray<FileSystemTreeItem> get_children();
	void clear_children();
	int get_index();
	...
	void move_before(FileSystemTreeItem *p_item);
	void move_after(FileSystemTreeItem *p_item);
...
VARIANT_ENUM_CAST(FileSystemTreeItem::TreeCellMode);

class FileSystemTree : public Control {
...
private:
	...
	FileSystemTreeItem *popup_pressing_edited_item = nullptr; // Candidate.
	int popup_pressing_edited_item_column = -1;

	FileSystemTreeItem *drop_mode_over = nullptr;
	...
	void update_item_cell(FileSystemTreeItem *p_item, int p_col) const;
	...
	struct Cache {
		...
		int click_index = -1;
		int click_id = -1;
		FileSystemTreeItem *click_item = nullptr;
		int click_column = 0;
	...
	Size2 _get_cell_icon_size(const FileSystemTreeItem::Cell &p_cell) const;
	...
	Rect2 search_item_rect(FileSystemTreeItem *p_from, FileSystemTreeItem *p_item);
	...
	int _count_selected_items(FileSystemTreeItem *p_from) const;
	bool _is_branch_selected(FileSystemTreeItem *p_from) const;
	...
	void _mouse_motion_input(const Ref<InputEventMouseMotion> &p_event);
...
public:
	...
	FileSystemTreeItem *create_item(FileSystemTreeItem *p_parent = nullptr, int p_index = -1);
	FileSystemTreeItem *get_root() const;
	FileSystemTreeItem *get_last_item() const;
	...
	int get_item_offset(FileSystemTreeItem *p_item) const;
	Rect2 get_item_rect(FileSystemTreeItem *p_item, int p_column = -1, int p_button = -1) const;
	...
	FileSystemTreeItem *search_item_text(const String &p_find, int *r_col = nullptr, bool p_selectable = false);
	// First item that matches the whole text, from the first item down.
	FileSystemTreeItem *get_item_with_text(const String &p_find) const;
	FileSystemTreeItem *get_item_with_metadata(const Variant &p_find, int p_column = -1) const;

	Point2 get_scroll() const;
	void scroll_to_item(FileSystemTreeItem *p_item, bool p_center_on_item = false);
