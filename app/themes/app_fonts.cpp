...
void app_register_fonts(const Ref<Theme> &p_theme) {
	Ref<DirAccess> dir = DirAccess::create(DirAccess::ACCESS_FILESYSTEM);

	TextServer::FontAntialiasing font_antialiasing = (TextServer::FontAntialiasing)(int)APP_GET("interface/app/font_antialiasing");
	int font_hinting_setting = (int)APP_GET("interface/app/font_hinting");
	TextServer::SubpixelPositioning font_subpixel_positioning = (TextServer::SubpixelPositioning)(int)APP_GET("interface/app/font_subpixel_positioning");
	bool font_disable_embedded_bitmaps = (bool)APP_GET("interface/app/font_disable_embedded_bitmaps");
	bool font_allow_msdf = (bool)APP_GET("interface/app/font_allow_msdf");
	...
	// Load built-in fonts.
	const int default_font_size = int(APP_GET("interface/app/main_font_size")) * APP_SCALE;
	...
	String custom_font_path = APP_GET("interface/app/main_font");
	String custom_font_path_bold = APP_GET("interface/app/main_font_bold");
	String custom_font_path_source = APP_GET("interface/app/code_font");
	...
	if (custom_font_path.length() > 0 && dir->file_exists(custom_font_path)) {
		...
		AppSettings::get_singleton()->set_manually("interface/app/main_font", "");
		default_fc->set_opentype_features(default_features);
		default_fc->set_base_font(default_font);
	}
	default_fc->set_spacing(TextServer::SPACING_TOP, -APP_SCALE);
	default_fc->set_spacing(TextServer::SPACING_BOTTOM, -APP_SCALE);
	...
	default_fc_msdf->set_spacing(TextServer::SPACING_TOP, -APP_SCALE);
	default_fc_msdf->set_spacing(TextServer::SPACING_BOTTOM, -APP_SCALE);
	...
	if (custom_font_path_bold.length() > 0 && dir->file_exists(custom_font_path_bold)) {
		...
		AppSettings::get_singleton()->set_manually("interface/app/main_font_bold", "");
		bold_fc->set_opentype_features(default_features);
		bold_fc->set_base_font(default_font_bold);
	}
	bold_fc->set_spacing(TextServer::SPACING_TOP, -APP_SCALE);
	bold_fc->set_spacing(TextServer::SPACING_BOTTOM, -APP_SCALE);
	...
	bold_fc_msdf->set_spacing(TextServer::SPACING_TOP, -APP_SCALE);
	bold_fc_msdf->set_spacing(TextServer::SPACING_BOTTOM, -APP_SCALE);
	bold_fc_msdf->set_variation_opentype(bold_fc_opentype);

	if (!String(APP_GET("interface/app/main_font_custom_opentype_features")).is_empty()) {
		Vector<String> subtag = String(APP_GET("interface/app/main_font_custom_opentype_features")).split(",");
		if (!subtag.is_empty()) {
			Dictionary ftrs;
	...
	if (custom_font_path_source.length() > 0 && dir->file_exists(custom_font_path_source)) {
		...
		AppSettings::get_singleton()->set_manually("interface/app/code_font", "");
		mono_fc->set_base_font(default_font_mono);
	}
	mono_fc->set_spacing(TextServer::SPACING_TOP, -APP_SCALE);
	mono_fc->set_spacing(TextServer::SPACING_BOTTOM, -APP_SCALE);

	Ref<FontVariation> mono_other_fc = mono_fc->duplicate();

	// Enable contextual alternates (coding ligatures) and custom features for the source app font.
	int ot_mode = APP_GET("interface/app/code_font_contextual_ligatures");
	switch (ot_mode) {
		...
		case 2: { // Custom.
			Vector<String> subtag = String(APP_GET("interface/app/code_font_custom_opentype_features")).split(",");
			Dictionary ftrs;
			for (int i = 0; i < subtag.size(); i++) {
				Vector<String> subtag_a = subtag[i].split("=");
				if (subtag_a.size() == 2) {
	...
	// Use fake bold/italics to style the app log's `print_rich()` output.
	...
	p_theme->set_font("main", AppStringName(AppFonts), default_fc);
	p_theme->set_font("main_msdf", AppStringName(AppFonts), default_fc_msdf);
	p_theme->set_font_size("main_size", AppStringName(AppFonts), default_font_size);

	p_theme->set_font("bold", AppStringName(AppFonts), bold_fc);
	p_theme->set_font("main_bold_msdf", AppStringName(AppFonts), bold_fc_msdf);
	p_theme->set_font_size("bold_size", AppStringName(AppFonts), default_font_size);

	p_theme->set_font("italic", AppStringName(AppFonts), italic_fc);
	p_theme->set_font_size("italic_size", AppStringName(AppFonts), default_font_size);

	// Title font.

	p_theme->set_font("title", AppStringName(AppFonts), bold_fc);
	p_theme->set_font_size("title_size", AppStringName(AppFonts), default_font_size + 1 * APP_SCALE);

	p_theme->set_type_variation("MainScreenButton", "Button");
	p_theme->set_font(SceneStringName(font), "MainScreenButton", bold_fc);
	p_theme->set_font_size(SceneStringName(font_size), "MainScreenButton", default_font_size + 2 * APP_SCALE);
	...
	p_theme->set_font_size(SceneStringName(font_size), "HeaderMedium", default_font_size + 1 * APP_SCALE);

	p_theme->set_type_variation("HeaderLarge", "Label");
	p_theme->set_font(SceneStringName(font), "HeaderLarge", bold_fc);
	p_theme->set_font_size(SceneStringName(font_size), "HeaderLarge", default_font_size + 3 * APP_SCALE);
	...
	// Documentation fonts
	p_theme->set_font_size("doc_size", AppStringName(AppFonts), int(APP_GET("text_editor/help/help_font_size")) * APP_SCALE);
	p_theme->set_font("doc", AppStringName(AppFonts), default_fc);
	p_theme->set_font("doc_bold", AppStringName(AppFonts), bold_fc);
	p_theme->set_font("doc_italic", AppStringName(AppFonts), italic_fc);
	p_theme->set_font_size("doc_title_size", AppStringName(AppFonts), int(APP_GET("text_editor/help/help_title_font_size")) * APP_SCALE);
	p_theme->set_font("doc_title", AppStringName(AppFonts), bold_fc);
	p_theme->set_font_size("doc_source_size", AppStringName(AppFonts), int(APP_GET("text_editor/help/help_source_font_size")) * APP_SCALE);
	p_theme->set_font("doc_source", AppStringName(AppFonts), mono_fc);
	p_theme->set_font_size("doc_keyboard_size", AppStringName(AppFonts), (int(APP_GET("text_editor/help/help_source_font_size")) - 1) * APP_SCALE);
	p_theme->set_font("doc_keyboard", AppStringName(AppFonts), mono_fc);

	// Ruler font
	p_theme->set_font_size("rulers_size", AppStringName(AppFonts), 8 * APP_SCALE);
	p_theme->set_font("rulers", AppStringName(AppFonts), default_fc);

	// Rotation widget font
	p_theme->set_font_size("rotation_control_size", AppStringName(AppFonts), 13 * APP_SCALE);
	p_theme->set_font("rotation_control", AppStringName(AppFonts), default_fc);

	// Code font
	p_theme->set_font_size("source_size", AppStringName(AppFonts), int(APP_GET("interface/app/code_font_size")) * APP_SCALE);
	p_theme->set_font("source", AppStringName(AppFonts), mono_fc);

	p_theme->set_font_size("expression_size", AppStringName(AppFonts), (int(APP_GET("interface/app/code_font_size")) - 1) * APP_SCALE);
	p_theme->set_font("expression", AppStringName(AppFonts), mono_other_fc);

	p_theme->set_font_size("output_source_size", AppStringName(AppFonts), int(APP_GET("run/output/font_size")) * APP_SCALE);
	p_theme->set_font("output_source", AppStringName(AppFonts), mono_other_fc);
	p_theme->set_font("output_source_bold", AppStringName(AppFonts), mono_other_fc_bold);
	p_theme->set_font("output_source_italic", AppStringName(AppFonts), mono_other_fc_italic);
	p_theme->set_font("output_source_bold_italic", AppStringName(AppFonts), mono_other_fc_bold_italic);
	p_theme->set_font("output_source_mono", AppStringName(AppFonts), mono_other_fc_mono);

	p_theme->set_font_size("status_source_size", AppStringName(AppFonts), default_font_size);
	p_theme->set_font("status_source", AppStringName(AppFonts), mono_other_fc);
}
