...
void EditorProperty::update_editor_property_status() {
	...
	if (can_pin) {
		...
		CRASH_COND(!node);
		// new_pinned = node->is_property_pinned(property);	// TODO
	}
...
Ref<Texture2D> EditorInspectorSection::_get_arrow() {
	Ref<Texture2D> arrow;
	if (foldable) {
		if (_is_section_unfolded(section)) {
...
void EditorInspectorSection::_set_section_unfold(const String &p_section, bool p_unfolded, bool p_initializing) {
	// if (!p_initializing) {
	// 	object->set_edited(true);
	// }

	if (p_unfolded) {
		section_folding.insert(p_section);
	} else {
		section_folding.erase(p_section);
	}
}

bool EditorInspectorSection::_is_section_unfolded(const String &p_section) {
	return section_folding.has(p_section);
}
	...
	switch (p_what) {
		...
		case NOTIFICATION_DRAW: {
			...
			// Draw header title, folding arrow and count of revertable properties.
			{
				...
				bool folded = (foldable || !checkbox_only) && !_is_section_unfolded(section);
...
void EditorInspectorSection::gui_input(const Ref<InputEvent> &p_event) {
	...
	if (k.is_valid() && k->is_pressed()) {
		if (foldable && can_click_unfold && k->is_action("ui_accept", true)) {
			...
			bool should_unfold = !_is_section_unfolded(section);
			if (should_unfold) {
				unfold();
			} else {
				fold();
	...
	if (mb.is_valid() && mb->is_pressed() && mb->get_button_index() == MouseButton::LEFT) {
		...
		if (checkable && check_rect.has_point(pos)) {
			...
			bool should_unfold = can_click_unfold && !_is_section_unfolded(section);
			if (should_unfold) {
				unfold();
			} else {
				fold();
...
void EditorInspectorSection::unfold() {
	...
	if (foldable) {
		_set_section_unfold(section, true);
	}
...
void EditorInspectorSection::fold() {
	...
	_set_section_unfold(section, false);
	vbox->hide();
	queue_redraw();
}
