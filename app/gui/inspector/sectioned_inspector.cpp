...
static bool _property_path_matches(const String &p_property_path, const String &p_filter, PropertyNameProcessor::Style p_style) {
	...
	for (int i = 0; i < sections.size(); i++) {
		if (p_filter.is_subsequence_ofn(PropertyNameProcessor::get_singleton()->process_name(sections[i], p_style, p_property_path))) {
			return true;
		}
	}
	return false;
...
void SectionedInspector::update_category_list() {
	...
	const PropertyNameProcessor::Style name_style = PropertyNameProcessor::get_settings_style();
	const PropertyNameProcessor::Style tooltip_style = PropertyNameProcessor::get_tooltip_style(name_style);
	...
	for (PropertyInfo &pi : pinfo) {
		...
		for (int i = 0; i < sc; i++) {
			...
			parent->set_custom_font(0, get_theme_font(SNAME("bold"), AppStringName(AppFonts)));
			...
			if (!section_map.has(metasection)) {
				...
				const String text = PropertyNameProcessor::get_singleton()->process_name(sectionarr[i], name_style, pi.name);
				const String tooltip = PropertyNameProcessor::get_singleton()->process_name(sectionarr[i], tooltip_style, pi.name);
...
ObjectInspector *SectionedInspector::get_inspector() {
	return inspector;
}
...
SectionedInspector::SectionedInspector() :
		...
		inspector(memnew(ObjectInspector)) {
	add_theme_constant_override("autohide", 1); // Fixes the dragger always showing up

	VBoxContainer *left_vb = memnew(VBoxContainer);
	left_vb->set_custom_minimum_size(Size2(190, 0) * APP_SCALE);
	add_child(left_vb);
	...
	right_vb->set_custom_minimum_size(Size2(300, 0) * APP_SCALE);
	right_vb->set_h_size_flags(SIZE_EXPAND_FILL);
	add_child(right_vb);

	inspector->set_v_size_flags(SIZE_EXPAND_FILL);
	right_vb->add_child(inspector, true);
	// inspector->set_use_doc_hints(true);	// TODO
