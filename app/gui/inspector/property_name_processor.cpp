#include "property_name_processor.h"

#include "app/app_modules/settings/app_settings.h"
#include "core/string/translation_server.h"

PropertyNameProcessor *PropertyNameProcessor::singleton = nullptr;

PropertyNameProcessor::Style PropertyNameProcessor::get_default_inspector_style() {
...
PropertyNameProcessor::Style PropertyNameProcessor::get_settings_style() {
	if (!EditorSettings::get_singleton()) {
...
PropertyNameProcessor::Style PropertyNameProcessor::get_tooltip_style(Style p_style) {
	return p_style == STYLE_LOCALIZED ? STYLE_CAPITALIZED : STYLE_LOCALIZED;
}

bool PropertyNameProcessor::is_localization_available() {
	return EditorSettings::get_singleton() && EDITOR_GET("interface/editor/editor_language") != "en";
}

String PropertyNameProcessor::_capitalize_name(const String &p_name) const {
...
StringName PropertyNameProcessor::_get_context(const String &p_name, const String &p_property, const StringName &p_class) const {
...
String PropertyNameProcessor::process_name(const String &p_name, Style p_style, const String &p_property, const StringName &p_class) const {
	switch (p_style) {
		case STYLE_RAW: {
			return p_name;
		} break;
...
String PropertyNameProcessor::translate_group_name(const String &p_name) const {
	if (TranslationServer::get_singleton()) {
